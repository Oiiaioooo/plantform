...
visa_address = 'USB0::0x0699::0x0368::C010936::INSTR'
...
while(True):
    ...
    bin_wave = scope.query_binary_values('curve?', datatype='b', container=np.array, chunk_size=1024 * 1024)
    ...
    voff = float(scope.query('wfmpre:yzero?'))  # reference_code voltage
    print('voff', voff)
    # YOFf是一个以数字化仪电平表示的值，用于使用以下公式将波形记录值转换为YUNit值（其中dl是数字化仪电平）：
    vpos = float(scope.query('wfmpre:yoff?'))  # reference_code position (level)
